...
public class CloudControllerFar : MonoBehaviour {
	...
	void Update () {
		...
		transform.position = Vector2.Lerp (transform.position, nextPosition, Time.deltaTime * 1f);
...
//		}
		if (transform.position.x > world.x + 2.2f) {
			transform.position  = new Vector2 (-(world.x + 1f), transform.position.y);
		}
	}
}
